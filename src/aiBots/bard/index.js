...
const chatMap = {
    '*': {
        ...
        preparePrompt: (messages, aiName) => `
            ...
            2. 回复需必须要以：${startTag} 开头，然后以 ${endTag} 结尾。
        `
    },
    '消遣娱乐': {
...
const prompt = async function (prompt) {
    ...
    await this.$eval('.text-input-field_textarea .textarea:nth-child(1)', (el, indexifyPrompt) => el.innerText = indexifyPrompt, indexifyPrompt);
